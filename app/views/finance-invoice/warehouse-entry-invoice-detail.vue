<template>
    <div id="warehouse-entry-invoice-detail">
        <gz-list
            ref="warehouseInvoiceDetail"
            :loading="loading"
            @getData="getListHanlder"
            :form="searchForm"
            :colNameMap="colNameMap"
            :originPaging="warehouseInvoiceSubDetail.paging"
            :originData="viewData"
            :pageShow="pageShow"
            >
            <el-form label-position="top" :model="searchForm" label-width="100px" slot="searchForm">
                <el-row :gutter="24">
                    ...
                    <el-col :span="6">
                        <el-form-item label="供应商：">
                            <span v-text="searchForm.fSupplierName"></span>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                ...
                <el-row :gutter="20">
                    <el-col :span="6">
                        <el-form-item label="入库总金额(元)：">
                            <span v-text="searchForm.inStockTotalMoney"></span>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="6">
                        <el-form-item label="关联采购单号：">
                            <span v-text="searchForm.purchaseId"></span>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
        ...
        <div class="button-handler">
            <el-button
                ...
                :disabled="warehouseInvoiceDetail.status !== 0"
                v-if="checkAuth('purchase-in-stock-invalid')"
                >作废</el-button>
            <el-button
                type="primary"
                @click="submitFinanceHandler"
                :disabled="warehouseInvoiceDetail.status !== 0"
                v-if="checkAuth('purchase-in-stock-send')"
                >提交财务</el-button>
...
export default {
    ...
    data() {
        return {
            ...
            searchForm: {
                ...
                fBizDate: '', // 入库日期
                fSupplierName: '', // 供应商名字
                fWarehouseName: '', // 入库仓库名字
                InStockOrderAmount: '', // 入库总金额
                confirmPerson: '', // 确认人
                auditDate: '', // 确认时间
                purchaseId: '', // 关联采购单号
                createDate: '', // 创建时间
    ...
    methods: {
        ...
        initData() {
            ...
            this.$refs.warehouseInvoiceDetail.queryCondition.paging.pageIndex = 0;
            this.$refs.warehouseInvoiceDetail.queryHandler();
        },
        getInvoiceDetail() {
            this.getWarehouseInvoiceDetail(this.$route.params.id)
            .then(() => {
                this.$set(this, 'searchForm', this.formatSearchFrom(this.warehouseInvoiceDetail));
            })
            .catch((err) => {
        ...
        getListHanlder(params) {
            this.loading = true;
            this.getWarehouseInvoiceSubDetail({
        ...
        // 转化getter里面的数据用于界面显示
        formatVieData() {
            return this.warehouseInvoiceSubDetail.itemList.map((item) => {
        ...
        invalidHandler() {
            this.$confirm('你确定要作废吗？', '提示', {
                ...
                // TODO 这里请求作废接口
                this.invalidWarehouseInvoice(this.warehouseInvoiceDetail.sysId)
                .then(() => {
        ...
        submitFinanceHandler() {
            ...
            this.$confirm('将数据提交给财务后不可更改，确认提交吗？', '提示', {
                ...
                // TODO 这里请求提交财务接口
                this.submitWarehouseInvoice(this.warehouseInvoiceDetail.sysId)
                .then(() => {
...
<style lang="less">
    #warehouse-entry-invoice-detail {
        .button-handler {
            padding-bottom: 10px;
            padding-top: 10px;
        }
