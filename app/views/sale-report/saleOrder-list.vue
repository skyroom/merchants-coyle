...
export default {
    ...
    data() {
        return {
            ...
            operation: [{
                text: '详情',
                key: 'detail',
                valiateFunc: () => this.checkAuth('seller-detail'),
            }],
            pageShow: true,
            loading: true,
            exportLoading: false,
    ...
    methods: {
        ...
        // 详情按钮点击触发函数
        clickToCheck(args) {
            this.$router.push(`/seller-detail/${args[1].id}`);
        },
        // 导出按钮点击触发函数
        exportHandler() {
            if (this.formDataSave()) return;
            this.exportLoading = true;
            this.getSaleOrderKey(this.thisCondition)
            .then(() => {
                this.downloadUrl = this.exportUrl(this.saleOrderKey);
            }).then(() => {
